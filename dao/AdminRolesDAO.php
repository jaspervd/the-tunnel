...
class AdminRolesDAO extends DAO {
	...
	public function update($id, $title, $can_create_admin, $can_approve_groups, $can_edit_creations, $can_delete_creations, $can_feature_creations, $can_judge_creations) {
		$sql = "UPDATE `tt_admin_roles` `title` = :title, `can_create_admin` = :can_create_admin, `can_approve_groups` = :can_approve_groups, `can_edit_creations` = :can_edit_creations, `can_delete_creations` = :can_delete_creations, `can_feature_creations` = :can_feature_creations, `can_judge_creations` = :can_judge_creations WHERE `id` = :id";
		$qry = $this->pdo->prepare($sql);
		$qry->bindValue(':id', $id);
		$qry->bindValue(':title', $title);
		$qry->bindValue(':can_create_admin', $can_create_admin);
